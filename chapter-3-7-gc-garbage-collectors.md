...
可能你會覺得這個collector很廢, 但到目前為止, 其仍然是JVM在client mode下預設的新生代collector. 畢竟相較於其他比較新的collector, serial collector有一個優點: 簡單且高效能\(當然是說跟其它collector的單執行緒相比\), 對於限定單CPU的環境來說, 此collector因為沒有執行緒互動的開銷, 所以專心做收垃圾這件事情自然可以獲得最高的單執行緒收集效率. 所以對於運作在client mode下的JVM來說, serial collector仍然是一個不錯的選擇.
